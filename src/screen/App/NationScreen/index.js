...
const NationScreen = () => {
    ...
    const getnationNews = async () => {
        try {
            setLoading(true);
            const response = await ACNetwork.get(
                `https://gnews.io/api/v4/top-headlines?category=nation&apikey=${APIKEY}`,
                (
                    await config()
                ).headers
            );

            setnationNews(response.data.articles);
